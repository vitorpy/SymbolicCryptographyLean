...
class LawfulMonadTransformer (t) [MonadTransformer t] : Prop where
  [monad_functor {m : _} [Monad m] [LawfulMonad m] : LawfulMonad (t m)]
  liftOf_pure {m : _} [Monad m] [LawfulMonad m] {α} (x : α) :
    liftOf m (pure x) = (pure x : t m α)
  liftOf_bind {m : _} [Monad m] [LawfulMonad m] {α β} (x : m α) (f : α → m β) :
